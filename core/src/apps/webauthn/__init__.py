...
class State:
    ...
    def boot(self, app_id: bytes) -> None:
...
class U2fState(State):
    ...
    def boot(self) -> None:
        super(U2fState, self).boot(self.app_id)
...
class Fido2State(State):
    def __init__(self, cid: int, client_data_hash: bytes, rp_id: str) -> None:
        ...
        self.app_name = rp_id
        self.boot(hashlib.sha256(rp_id).digest())
...
class Fido2ConfirmGetAssertion(Fido2State):
    def __init__(
        ...
        super(Fido2ConfirmGetAssertion, self).__init__(cid, client_data_hash, creds[0].rp_id)
        self.creds = creds
...
class Fido2ConfirmExcluded(Fido2State):
    ...
    def get_dialog(self):
        ...
        text.normal("This token is already", "registered with", self.app_name + ".")
    ...
    if user_verification and not config.has_pin():
        ...
        state_set = dialog_mgr.set_state(
            Fido2ConfirmGetAssertion(
                req.cid, client_data_hash, cred_list, hmac_secret
            )
        )
    if not state_set:
        return cmd_error(req.cid, _ERR_CHANNEL_BUSY)
